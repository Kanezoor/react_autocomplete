...
describe('Page', () => {
  ...
  describe('initial state', () => {
    it('should display the initial page title', () => {
      page.title().should('contain', 'No selected person');
    });
    ...
    it('should not render "No matching suggestions"', () => {
      page.noSuggestionsMessage().should('not.exist');
    });
  });
  ...
  describe('user interactions', () => {
    it('should display the list of all people when input is focused but empty', () => {
      ...
      page.suggestionsList().should('be.visible');
    ...
    it('should display "No matching suggestions" for non-matching input', () => {
      page.searchInput().type('non-matching');
    ...
    it('should display suggestions for matching input', () => {
      ...
      page.searchInput().type(typedName);
    ...
    it('should update the title with selected person', () => {
      ...
      page.searchInput().type(person.name);

      page.waitForDebounce();

      page.suggestionItems().contains(person.name).click();

      page.title().should('contain', page.getPageTitle(person));
    });
    ...
    it('should update the title after another person is selected', () => {
      ...
      page.searchInput().type(person1.name);

      page.waitForDebounce();

      page.suggestionItems().contains(person1.name).click();

      page.searchInput().clear();

      page.searchInput().type(person2.name);

      page.waitForDebounce();

      page.suggestionItems().contains(person2.name).click();

      page.title().should('contain', page.getPageTitle(person2));
    });
    ...
    it('should set the title to "No selected person" after input is changed', () => {
      ...
      page.searchInput().type('{backspace}');
